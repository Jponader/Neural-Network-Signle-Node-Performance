...
filter2 = np.array([[1,1],[2,1]])
filter3 = np.array([[2,1],[3,1]])
filter4 = np.array([[3,1],[1,3]])

inputlen = len(inputMat)
filterLen = len(filter1)
solution = convolve(inputMat, filter1, mode="valid")
solution2 = convolve(inputMat, filter2, mode="valid")
solution3 = convolve(inputMat, filter3, mode="valid")
solution4 = convolve(inputMat, filter4, mode="valid")
size = len(solution)
...
print(np.linalg.cond(interstep))

#inverse = np.linalg.inv(interstep)
np.random.shuffle(interstep)
print(interstep)
